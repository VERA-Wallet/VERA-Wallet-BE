...
export class FrontendEventCommandController {
  constructor(private readonly commands: EventReclassificationService, private readonly indexer: IndexerService) {}
  // Sole user-visible incremental trigger. Forced all-binding refresh; reachable via the /api/events/* proxy wildcard.
  @Post("resync") async resync(@CurrentUser() user: AuthenticatedUser) { return success(await this.indexer.sync(user.sub)); }

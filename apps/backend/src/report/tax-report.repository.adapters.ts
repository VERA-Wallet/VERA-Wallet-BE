...
import type { TaxRepository } from "../tax/tax.repository";
import type { ReportRepository } from "./report.repository";

@Injectable()
export class MockTaxReportRepository implements TaxRepository, ReportRepository {
  ...
  async listEvents(userId: string, countryCode?: string) {
    const reportIds = new Set([...this.reports.values()].filter((report) => report.userId === userId).map((report) => report.id));
    return [...this.events.values()].filter((event) => event.reportId !== null && reportIds.has(event.reportId) && (!countryCode || event.countryCode === countryCode));
  }
...
@Injectable()
export class PrismaTaxReportRepository implements TaxRepository, ReportRepository {
  constructor(private readonly prisma: PrismaService) {}

  async createReport(userId: string, period: string, countryCode: string, totalGain: string, events: CalculatedTaxEvent[]) {
    return this.prisma.$transaction(async (transaction) => {
      const report = await transaction.taxReport.create({ data: { userId, period, countryCode, totalGain: new Prisma.Decimal(totalGain), status: "draft" } });
      await transaction.taxEvent.createMany({ data: events.map((event) => ({ txId: event.transactionId, reportId: report.id, countryCode, ruleVersion: event.ruleVersion, gainLoss: new Prisma.Decimal(event.gainLoss), isEstimate: true })) });
